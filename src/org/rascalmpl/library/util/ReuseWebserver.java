...
public class ReuseWebserver {
  ...
  public ReuseWebserver(IValueFactory vf) {
    this.vf = vf;
  ...
  public void serve(ISourceLocation url, final IValue callback, final IEvaluatorContext ctx) {
	  if(!this.servers.containsKey(url)) {
		  this.GetInstance(url, callback, ctx);
	  }
  }
  
  private void GetInstance(ISourceLocation url, final IValue callback, final IEvaluatorContext ctx)  
  {
      URI uri = url.getURI();
  ...
  private void initMethodAndStatusValues(final IEvaluatorContext ctx) {
    if (methodValues.isEmpty() || statusValues.isEmpty()) {
      Environment env = ctx.getHeap().getModule("util::ReuseWebserver");
